...
const Media2 = ()=>{
  const {userId} = useContext(userType);
    ...
    const das = localStorage.getItem("image1");
...
formData.append('userId',userId);// Assuming 'file' is the File object you want to upload
 ...
 if(das==null){ await axios.post('http://127.0.0.1:5000/images', formData).then(response => {
  // Handle response
  alert("upload image successfully");
  setOpen1(false);
  localStorage.setItem("image1",1);
  ...
  await axios.put(`http://127.0.0.1:5000/images/${userId}`, formData).then(response => {
  // Handle response
  alert("upload image successfully");
  setOpen1(false);
